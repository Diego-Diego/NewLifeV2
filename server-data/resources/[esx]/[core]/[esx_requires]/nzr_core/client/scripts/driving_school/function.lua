	...
	while true do
		...
		if timer_active then
			...
			if ConfigAutoEcole.timer > 0 then
				...
				ESX.ShowNotification(("Il vous restes ~r~%s~s~ secondes."):format(ConfigAutoEcole.timer))
				Wait(1000)
			end
			if ConfigAutoEcole.timer == 0 then
				ESX.ShowNotification("[~r~Échoué~s~] vous avez mit trop de temps à remplir l'examen")
				DoScreenFadeOut(1700)
				Wait(2000)
				interval = 500
				timer_active = false
